...
const CHATBOT_MANUAL={
  home:`
[홈 화면]
- 신규 고객 추가: 우측 상단 [+ 신규 고객 등록] 버튼 클릭
- 고객 컨설팅 화면 이동: 고객 카드 클릭
- 고객 삭제: 고객 카드 우측 [✕] 버튼 클릭 → 확인창에서 확인
- 단계별 색상 구분: Discovery=파란색, Diagnosis=초록색, Build=보라색
- 진행률: 카드 우측에 완료 퍼센트(%) 및 바 표시
- 로그아웃: 우측 상단 [🔒 로그아웃] 버튼 클릭
  `,
  "0-1":`
[Phase 0 Step 1 — 사전 준비]
- AI 사전조사 실행: 고객명·업종 입력 후 [🔍 AI 자동 사전 조사 실행] 버튼 클릭 → 업종 특성·트렌드·예상 Pain Point 자동 생성
- 직접 수집 정보 입력: "직접 수집 정보 입력" 텍스트 영역에 네이버 플레이스·SNS 정보 입력 → 자동 저장
- 빠른 입력 태그: [네이버 플레이스] [SNS 분석] [경쟁사 정보] [지인 정보] 버튼 클릭 → 텍스트 영역에 태그 자동 삽입
- 가설 Pain Point 선택: 업종별 태그 클릭으로 선택/해제 (복수 선택 가능)
- 인터뷰 질문지 생성: [✨ 인터뷰 질문지 자동 생성] 버튼 클릭 → 질문지 자동 작성
- 질문지 복사: [📋 질문지 복사] 버튼 클릭 → 클립보드 복사
- 질문지 재생성: [🔄 재생성] 버튼 클릭
- 다음 Step 이동: 체크리스트 항목 클릭으로 완료 처리 후 [준비 완료 → 아이스브레이킹 →] 버튼 클릭
  `,
  "0-2":`
[Phase 0 Step 2 — 아이스브레이킹]
- 체크리스트 완료: 각 항목 클릭으로 완료/취소 처리
- 메모 입력: "특이사항 / 메모" 텍스트 영역에 자유롭게 입력 → 자동 저장
- 다음 Step 이동: [아이스브레이킹 완료 → 현황 인터뷰 →] 버튼 클릭
- 이전 Step 이동: [← 이전] 버튼 클릭
  `,
  "0-3":`
[Phase 0 Step 3 — 현황 인터뷰]
- 답변 직접 입력: Q1·Q2·Q3 텍스트 영역에 고객 답변 직접 입력
- STT 음성 변환: [🎤 음성 파일 업로드 (STT)] 버튼 클릭 → mp3 파일 선택 → Whisper가 텍스트 변환 후 Q1~Q3 자동 입력
- 🤖 배지 표시: AI가 자동 입력한 항목에 배지 표시 → 텍스트 영역 클릭으로 직접 수정 가능
- 추가 메모: "기타 / 추가 메모" 텍스트 영역에 자유 입력
- 체크리스트 완료: 각 항목 클릭으로 완료 처리
- 다음 Step 이동: [인터뷰 완료 → AI 분석 →] 버튼 클릭
  `,
  "0-4":`
[Phase 0 Step 4 — AI 분석]
- Pain Point 자동 도출: [🔍 Pain Point AI 분석] 버튼 클릭 → 인터뷰 내용 기반 Pain Point 카드 자동 생성
- Pain Point 수정: 카드 내 제목·유형·영향도·해결방향 텍스트 영역 클릭 후 직접 수정
- Pain Point 추가: 하단 [＋ Pain Point 추가] 버튼 클릭 → 빈 카드 추가
- Pain Point 삭제: 카드 우측 상단 [✕] 버튼 클릭
- 다음 Step 이동: [분석 완료 → 확정 & 전달 →] 버튼 클릭
  `,
  "0-5":`
[Phase 0 Step 5 — 확정 & 전달]
- 확정 체크리스트 완료: 각 항목 클릭으로 완료 처리
- Phase 1 이동: [Discovery 완료 → Diagnosis →] 버튼 클릭
- 이전 Step 이동: [← 이전] 버튼 클릭
  `,
  "1-1":`
[Phase 1 Step 1 — 문제 재확인]
- Phase 0 Pain Point 목록 확인: 화면 상단 카드에 자동 표시
- 추가 Pain Point 입력: "추가 Pain Point" 텍스트 영역에 직접 입력
- 재확인 체크리스트 완료: 각 항목 클릭으로 완료 처리
- 다음 Step 이동: [재확인 완료 → 솔루션 설계 →] 버튼 클릭
  `,
  "1-2":`
[Phase 1 Step 2 — 솔루션 설계]
- 예산 선택: "예산 범위" 드롭다운 클릭 후 항목 선택
- 일정 선택: "희망 일정" 드롭다운 클릭 후 항목 선택
...
- 다음 Step 이동: [솔루션 확정 → 실현 가능성 →] 버튼 클릭
  `,
  "1-3":`
[Phase 1 Step 3 — 실현 가능성]
- 실현 가능성 체크: 솔루션별 체크리스트 항목 클릭으로 완료 처리
- AI 평가 실행: 솔루션별 [🤖 AI 실현 가능성 평가] 버튼 클릭 → 강점·리스크·성공조건·권고 자동 생성
- 공통 리스크 메모: "공통 리스크 메모" 텍스트 영역에 입력
- 권고 사항 입력: "반영할 권고 사항" 텍스트 영역에 AI 평가 결과 복사 후 입력
- 반영 항목 선택: 체크박스 클릭으로 제안서 반영 항목 선택
- 제안서 재생성: [🔄 선택한 권고 사항 반영 → 제안서 재생성] 버튼 클릭
- 다음 Step 이동: [평가 완료 → 제안서 최종 확인 →] 버튼 클릭
  `,
  "1-4":`
[Phase 1 Step 4 — 제안서 최종 확인]
- 제안서 직접 편집: 제안서 텍스트 영역 클릭 후 직접 수정
- 전체 복사: [📋 전체 복사] 버튼 클릭 → 클립보드 복사
- Step 2로 돌아가기: [← STEP 2에서 재생성] 버튼 클릭
- Step 3으로 돌아가기: [← STEP 3에서 권고 반영] 버튼 클릭
- 완료 체크리스트: 각 항목 클릭으로 완료 처리
- 다음 Step 이동: [제안서 완료 → 발표 & 컨펌 →] 버튼 클릭
  `,
  "1-5":`
[Phase 1 Step 5 — 발표 & 컨펌]
- 반론 빠른 선택: "비용이 부담돼요" 등 태그 클릭 → 반론 텍스트 자동 입력
- 반론 직접 입력: "고객 반론 입력..." 텍스트 영역에 직접 입력
- AI 대응 답변 생성: [🤖 AI 대응 답변] 버튼 클릭 → 반론 대응 스크립트 자동 생성
- 컨펌 체크리스트 완료: 각 항목 클릭으로 완료 처리
- 계약 메모 입력: "착수금, 일정, 특이사항..." 텍스트 영역에 입력
- Phase 2 이동: [✓ Diagnosis 완료 → Build →] 버튼 클릭
  `,
  "2-1":`
[Phase 2 Step 1 — 개발 착수 Agile PM]
- 프로젝트 플랜 자동생성: [⚡ AI 프로젝트 플랜 자동생성] 버튼 클릭 → 스프린트·태스크 자동 생성
- 스프린트 보드 — 태스크 상태 변경: 태스크 카드 하단 [→진행중] [→완료] [→보류] 버튼 클릭
- 스프린트 보드 — 태스크 삭제: 태스크 카드 우측 상단 [✕] 버튼 클릭
...
- MVP 체크리스트 완료: 각 항목 클릭으로 완료 처리
- AI 개발 조언 생성: [🤖 AI 개발 조언] 버튼 클릭 → 모듈별 구현순서·리스크·MVP 범위·사전준비사항 자동 생성
- 개발 메모 입력: "개발 메모, 이슈..." 텍스트 영역에 자유 입력
- 다음 Step 이동: [MVP 완료 → 파일럿 →] 버튼 클릭
- 이전 Step 이동: [← 이전] 버튼 클릭
  `,
  "2-3":`
[Phase 2 Step 3 — 파일럿 테스트]
- 파일럿 체크리스트 완료: 각 항목 클릭으로 완료 처리
- 피드백 메모 입력: "고객 피드백 내용..." 텍스트 영역에 입력
- AI 사용 설명서 생성: [✨ AI 사용 설명서 생성] 버튼 클릭 → 고객용 설명서 자동 작성
- 설명서 편집: 설명서 텍스트 영역 클릭 후 직접 수정
- 설명서 복사: [📋 복사] 버튼 클릭 → 클립보드 복사
- 다음 Step 이동: [파일럿 완료 → 이관 →] 버튼 클릭
  `,
  "2-4":`
[Phase 2 Step 4 — 이관 & 완료]
- 이관 체크리스트 완료: 각 항목 클릭으로 완료 처리 (5개 항목 모두 완료 시 완료 확정 가능)
- AI 케이스 스터디 작성: [✨ AI 케이스 스터디 작성] 버튼 클릭 → 케이스 스터디 자동 생성
- 케이스 스터디 편집: 텍스트 영역 클릭 후 직접 수정
- 케이스 스터디 복사: [📋 복사] 버튼 클릭 → 클립보드 복사
- 프로젝트 완료 확정: 체크리스트 5개 모두 완료 후 [🎉 프로젝트 완료 확정] 버튼 클릭
  `,
  default:`
[시스템 공통]
- 체크리스트 완료: 각 항목 클릭으로 완료/취소 처리
- 다음 Step 이동: 화면 하단 오른쪽 버튼 클릭 (체크리스트 완료 불필요)
