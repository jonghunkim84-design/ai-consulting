...
const D_AN_EXAMPLE="\n\n[출력 예시 — 형식과 구체성 수준만 참고. 아래 내용을 그대로 베끼지 말고 실제 입력 내용에 맞게 새로 작성하라]\n{\"painPoints\":[{\"rank\":1,\"title\":\"수작업 접수 대장으로 인한 분실·오배송 반복\",\"type\":\"반복업무자동화\",\"impact\":\"월 3~4건 고객 클레임 발생, 재작업 인건비 손실\",\"solution\":\"카카오톡 챗봇 접수+QR 태그 자동 매칭 시스템\"},{\"rank\":2,\"title\":\"성수기 가격 문의 전화 응대에 하루 2시간 소요\",\"type\":\"고객응대자동화\",\"impact\":\"사장님 실제 작업 시간 감소, 대기 고객 발생\",\"solution\":\"자동 응답 챗봇으로 기본 가격표·소요시간 안내 자동화\"}],\"summary\":\"수작업 기반 접수·응대 프로세스가 반복 업무 부담과 고객 불만의 핵심 원인으로 확인됨.\",\"nextAction\":\"접수 자동화 솔루션 우선 제안, 2차 방문 시 예산 범위 확인 필요\"}\n";

const D_SOL_EXAMPLE="\n\n[출력 예시 — 형식과 구체성 수준만 참고. 아래 내용을 그대로 베끼지 말고 실제 입력 내용에 맞게 새로 작성하라. 실제로는 반드시 5개를 모두 작성할 것]\n{\"solutions\":[{\"rank\":1,\"title\":\"카카오톡 챗봇 접수·픽업 알림 시스템\",\"type\":\"반복업무자동화\",\"desc\":\"고객이 카톡으로 접수하면 QR 태그 자동 발급, 완료 시 알림 자동 발송\",\"why\":\"수작업 대장 기반 분실·오배송을 데이터로 추적 가능하게 함\",\"tool\":\"카카오 채널+Make(구 Integromat)\",\"effort\":\"2~3주\",\"cost\":\"초기 구축 30~50만원, 월 유지비 2~3만원\"},{\"rank\":2,\"title\":\"AI 가격 문의 자동 응답 챗봇\",\"type\":\"고객응대자동화\",\"desc\":\"세탁물 종류·수량 입력하면 예상 가격·소요시간 즉시 안내\",\"why\":\"전화 응대 시간을 줄여 사장님이 작업에 집중 가능\",\"tool\":\"ChatGPT API+카카오톡 채널\",\"effort\":\"1~2주\",\"cost\":\"월 3~5만원(API 사용량 기준)\"}]}\n";
...
export default function App(){
  ...
  if(view==="home") return <div style={{maxWidth:760,margin:"0 auto",padding:"1rem 0",fontFamily:"var(--font-sans)"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:"1.5rem"}}>
      {PHASES.map(p=><div key={p.id} style={{background:p.bg,border:`0.5px solid ${p.color}30`,borderRadius:12,padding:"20px 16px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center"}}>
        <div style={{fontSize:13,fontWeight:600,color:p.color,letterSpacing:"0.04em",marginBottom:6}}>Phase {p.id}</div>
        <div style={{fontSize:22,fontWeight:700,lineHeight:1.2,marginBottom:8}}>{p.label}</div>
  ...
  return <div style={{maxWidth:760,margin:"0 auto",padding:"1rem 0",fontFamily:"var(--font-sans)"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:"1.25rem"}}>
      {PHASES.map(p=>{const ia=active.phase===p.id,id=active.phasesDone[p.id],pc2=pc(p.id),pb2=pb(p.id);
        return <div key={p.id} onClick={()=>{if(id||ia)upd({phase:p.id,step:0,status:p.key});}} style={{borderRadius:10,padding:"20px 16px",background:id||ia?pb2:"var(--color-background-secondary)",border:`${ia?"2px":"0.5px"} solid ${ia||id?pc2+"50":"var(--color-border-tertiary)"}`,opacity:!ia&&!id?0.5:1,cursor:id||ia?"pointer":"default",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center"}}>
          <div style={{fontSize:13,color:pc2,fontWeight:600,letterSpacing:"0.04em",marginBottom:6}}>Phase {p.id}</div>
          <div style={{fontSize:22,fontWeight:700,lineHeight:1.2,marginBottom:8}}>{p.label}</div>
          {id&&<div style={{fontSize:13,color:pc2,fontWeight:500}}>✓ 완료</div>}
          {ia&&!id&&<div style={{fontSize:13,color:pc2,fontWeight:500}}>{p.steps[active.step]}</div>}
        </div>;
      })}
    </div>
    {/* Step 바 */}
    ...
    {active.phase===0&&<>
      ...
      {active.step===2&&<>
        ...
        {active.interviewQ&&<Panel title="생성된 질문지 참고" icon="📋" accent={C.blueBg}>
          ...
          <div style={{fontSize:12,lineHeight:1.7,whiteSpace:"pre-wrap",color:"var(--color-text-secondary)",maxHeight:180,overflowY:"auto",background:"var(--color-background-primary)",borderRadius:8,padding:"10px 12px"}}>{typeof active.interviewQ==="object"&&active.interviewQ?["[ 필수 질문 ]",...(active.interviewQ.essentialQuestions?.map((q,i)=>`${i+1}. ${q}`)??[]),"","[ 심화 질문 ]",...(active.interviewQ.deepDiveQuestions?.map((q,i)=>`${i+1}. ${q}`)??[]),"","[ 현장 팁 ]",...(active.interviewQ.fieldTips?.map(t=>`• ${t}`)??[])].join("\n"):active.interviewQ}</div>
        </Panel>}
